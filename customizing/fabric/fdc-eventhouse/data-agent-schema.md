...
| 센서 | 설비 유형 | 정상 범위 | 단위 |
|---|---|---|---|
| `CHAMBER_PRESSURE` | Etcher | 42~48 | **mTorr** |
| `CHAMBER_PRESSURE` | CVD | 4.6~5.4 | **Torr** |
| `CHAMBER_TEMP` | Furnace | 1040~1060 | degC |
| `CHAMBER_TEMP` | CVD | 410~430 | degC |
| `CHAMBER_TEMP` | Etcher | 62~68 | degC |

압력은 1000배 차이라 단위를 보면 알아챕니다. **온도는 셋 다 `degC` 입니다.**
420이 1040~1060 규격에 걸리면 그냥 심각한 저온으로 보입니다. 뒤집힌 판정의
78.2%가 이렇게 단위가 같은 경우라 의심할 단서가 없습니다.

### 판정에는 조인이 필요 없습니다

`fdc_sensor_reading.status` 에 `Normal` / `Warning` / `Alarm` 이 이미 들어 있습니다.
그 설비 유형의 한계치로 계산해 둔 값입니다. **"경보가 몇 건인가"에 답할 때 스펙과
조인하지 마세요.** `unit` 도 판독 행에 있습니다.

한계치의 **수치 자체**가 필요할 때만 조인하고, 그때는 반드시 `eqp_type` 을 함께
거세요.
