...
**`sensor_code` 만으로 조인하면 결과가 3.6배로 부풀고 그중 72.2%가 엉뚱한 설비
유형의 한계치와 짝지어집니다.** 에러는 나지 않습니다. 행 수와 집계값만 틀립니다.

같은 코드가 여러 설비 유형에 있으면 `inner join` 이 **모든 짝을 만듭니다.** 판독값
하나가 여러 행이 되므로 `count()` 나 `avg()` 가 통째로 어긋납니다. 판독값의 53.9%가
이렇게 부풀려집니다. 잘못된 짝으로 정상/경보를 다시 판정하면 **9.2%에서 판정이
뒤집힙니다.**

### 무엇이 부풀리고 무엇이 뒤집는가

두 피해가 서로 다른 센서에서 나옵니다.

| 센서 | 걸친 유형 | 부풀림 기여 | 판정 뒤집힘 | 단위로 알아챌 수 있나 |
|---|---|---|---|---|
...
챔버 센서 둘만 판정을 뒤집습니다. 그중 `CHAMBER_TEMP` 가 78.2%인데 **단위가 셋 다
`degC` 라 단서가 없습니다.**

| 설비 유형 | 정상 범위 | 단위 |
|---|---|---|
| Furnace | 1040~1060 | degC |
| CVD | 410~430 | degC |
| Etcher | 62~68 | degC |

420이 1040~1060 규격에 걸리면 그냥 심각한 저온으로 보입니다. 값도 단위도
그럴듯합니다. `CHAMBER_PRESSURE` 는 1000배 차이라 단위를 보면 알아챕니다.

### 판정에는 조인이 필요 없습니다

`fdc_sensor_reading.status` 에 `Normal` / `Warning` / `Alarm` 이 이미 들어 있습니다.
그 설비 유형의 한계치로 계산해 둔 값입니다. **"경보가 몇 건인가"에 답할 때 스펙과
조인하지 마세요.** `unit` 과 `eqp_type` 도 판독 행에 있습니다.

### 조인이 필요한 질문도 있습니다

스펙 10개 컬럼 중 판독 행에 복제된 것은 `sensor_code` · `eqp_type` · `unit` 셋뿐입니다.
나머지는 조인해야 합니다.

`sensor_name_ko` · `normal_min` · `normal_max` · `alarm_min` · `alarm_max` ·
`sample_interval_sec` · `is_active`

"한계까지 여유가 얼마인가", "이 센서의 한글 이름이 무엇인가", "표본 주기가 어떻게
되는가"는 조인이 필요합니다. **그때는 반드시 `eqp_type` 을 함께 거세요.**
