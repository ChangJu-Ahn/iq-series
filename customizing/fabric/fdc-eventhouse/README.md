...
| | 원본 | +37시간 | +11일 | +30일 |
|---|---|---|---|---|
| 판독 행 | 102,552 | 102,552 | 102,552 | 102,552 |
| Run / Idle | 93,210 / 9,342 | 93,210 / 9,342 | 93,210 / 9,342 | 93,210 / 9,342 |
| **Alarm 행** | **294** | **257** | **294** | **294** |
| **경보 난 설비·로트 짝** | **5** | **5** | **5** | **5** |
| 원본과 값이 다른 행 | — | **102,540** | **0** | **0** |

행 수도 가동 배치도 완전히 같습니다. MES 런 구조가 불변이기 때문입니다. **그런데
하루의 정수배가 아니게 옮기면 10만 행이 전부 다른 값이 됩니다.** 경보 짝은 다섯
개로 같지만 그 안의 설비가 갈립니다 — 원본은 PHOT02, +37시간은 TEST01입니다.

환경 성분이 하루 주기라서 그렇습니다. 밤에 돌던 공정이 새벽 공정이 되면 기저
온도가 이동하고, 경보 임계를 넘나드는 지점이 바뀝니다. **물리적으로 맞는 동작이라
고칠 것이 아닙니다.** 정수 일수 이동에서 값 차이가 0인 것이 그 증거입니다 —
같은 시각에 도는 공정은 같은 환경을 봅니다.
...
현재 배포된 MES 는 `2026-06-13T00:00:00Z` 로 고정돼 있습니다. 20초 간격으로 두 번
확인했고 움직이지 않았습니다. 이 저장소의 테스트 픽스처는 앵커가 `2026-09-04T00:00:00Z`
로 83일 늦지만 **하루의 정수배라 판독값이 102,552행 전부 일치합니다.** 값·상태·경보
설비가 모두 같고 시각만 정확히 83일 밀립니다. 참가자가 실제 MES 로 돌린 결과와
저희가 테스트한 결과가 같은 데이터라는 뜻입니다.

픽스처를 라이브 앵커로 갈지 않은 것은 의도한 것입니다. 갈아 버리면 "정수 일수
이동에는 값이 변하지 않는다"를 실증하는 대조군이 사라집니다. 앵커가 다른 두 벌을
들고 있어야 그 불변식이 실제로 성립하는지 매번 재볼 수 있습니다.
