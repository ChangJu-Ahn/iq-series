...
### ⚠️ MES를 앵커 없이 배포하면 시간축이 계속 움직입니다

MES는 SQLite를 `EmptyDir` 볼륨에 두고 0개까지 축소되므로, 콜드스타트할 때마다
시드를 다시 돌립니다. `MES_ANCHOR` 가 박혀 있으면 몇 번을 재시드해도 같은
데이터가 나옵니다. **없으면 앵커가 재시작 시각으로 잡혀 공정이력 전체가 통째로
평행이동합니다.** 실제 배포에서 20분 만에 움직이는 것을 관측했습니다.

이게 조용한 실패입니다. 이 노트북은 watermark 이후만 만들기 때문에 앵커가
움직여도 행이 겹치지 않고, 중복 검사도 정합 검증도 통과합니다. 한 테이블 안에
서로 다른 시간축의 조각이 쌓일 뿐입니다.

| 어긋남 | 맞게 찾음 | 못 찾음 | **틀린 로트를 답함** |
|---|---|---|---|
| 30분 | 96% | 4% | 0% |
| 60분 | 53% | 47% | 0% |
| **4시간** | 0% | 62% | **38%** |

런 길이가 34~166분이라 30분까지는 대부분 살아남지만, 네 시간을 넘으면 에이전트가
**틀린 답을 확신 있게** 내놓습니다. 앵커가 네 시간 이동한 뒤 한 번만 더 적재해도
테이블의 40%가 라이브 MES와 모순됐습니다.

### 그리고 참가자마다 경보 설비가 달라집니다

한 사람 안에서 시간축이 섞이는 것과 별개로, **20명이 서로 다른 데이터를 봅니다.**
공정이력을 통째로 옮겨 실측했습니다.

| | 원본 | +37시간 | +11일 |
|---|---|---|---|
| 판독 행 | 102,552 | 102,552 | 102,552 |
| Run / Idle | 93,210 / 9,342 | 93,210 / 9,342 | 93,210 / 9,342 |
| **Alarm 행** | **301** | **240** | **310** |
| **경보 난 설비·로트 짝** | **7** | **6** | **6** |

행 수도 가동 배치도 완전히 같습니다. MES 런 구조가 불변이기 때문입니다. **그런데
경보가 난 설비 자체가 달라집니다.**

환경 성분이 하루 주기라서 그렇습니다. 밤에 돌던 공정이 새벽 공정이 되면 기저
온도가 이동하고, 경보 임계를 넘나드는 지점이 바뀝니다. 물리적으로는 맞는
동작이지만 실습에서는 이렇게 나타납니다.

```
강사   "IMPL01에서 경보가 났죠?"
참가자A  네
참가자B  아니요, 저는 CVD01인데요
```

**앵커 고정은 선택이 아닙니다.** 고정하면 20명 전원이 같은 데이터를 받고, 몇 번을
재시작해도 그대로입니다.

### 앵커가 움직였을 때 복구

노트북이 이걸 감지해 멈춥니다. 멈추면 이렇게 하세요.

```bash
# 1. 앵커를 명시해 MES 재배포 (이후로는 재시작해도 데이터가 그대로입니다)
az deployment group create -g <rg> -f infra/main.bicep \
    -p mesAnchor='2026-09-15T00:00:00Z'

# 이미 시간축 코드가 배포돼 있다면 환경변수만 세팅해도 됩니다
az containerapp update -n mock-mes -g <rg> \
    --set-env-vars MES_ANCHOR=2026-09-15T00:00:00Z
```

```kusto
// 2. 어긋난 시간축이 섞인 테이블을 비우고 처음부터 다시 적재
.drop table fdc_sensor_reading
.drop table fdc_sensor_spec
```

QMS도 같은 MES를 봅니다. **FDC와 QMS를 서로 다른 앵커에서 적재하면** 각자는
내부 정합이 맞아도 `eqp_id` 로 두 시스템을 건너다니는 시나리오가 어긋납니다.
재배포 후에는 양쪽을 함께 다시 적재하세요.
...
실제로 재보면 이렇습니다. `EQP-ETCH01` 이 유휴이던 틈에 60분짜리 실적을 하나
등록하고 노트북을 다시 돌린 결과입니다.

| | `Run` 행 | `Idle` 행 |
|---|---:|---:|
| Eventhouse 최종 | **0** | 24 |
| 등록 후 처음부터 백필했다면 | **720** | 0 |

720행이 통째로 없습니다. 그런데 어디에도 경고가 뜨지 않습니다.
...
노트북은 실행마다 **새 프로세스**로 뜨므로 `hash()` 를 쓰면 백필한 구간과 라이브로
