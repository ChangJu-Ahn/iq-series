...
**열화 로트는 산포가 넓습니다.** 검사가 `조건부합격`이면서 부적합이 있으면, 그 검사의 측정값은 정상 로트보다 **2.2배** 흩어지도록 생성됩니다. 설비가 열화된 상태를 뜻합니다. "조건부합격 로트의 계측이 왜 흔들리는가" 같은 질문이 이 위에서 성립합니다. 규격을 벗어나지 않은 값도 많으므로 `is_out_of_spec` 만 세면 이 차이가 보이지 않습니다. 목표값 대비 편차(`deviation_pct`)의 산포를 보세요.
...
```
틀린 짝 5,628건 중 판정 뒤집힘 432 (7.7%)
  그중 단위까지 같아 알아챌 단서가 없는 것 432 (100%)
```
...
| CD | 16행 | 640 | 432 | 판정 왜곡 |
| RS | 16행 | 624 | 0 | 집계만 왜곡 |
| OVL | 8행 | 160 | 0 | 집계만 왜곡 |
| WRP | 8행 | 136 | 0 | 집계만 왜곡 |

부풀림 5,888건 중 **5,248건(89%)이 판정을 하나도 안 뒤집습니다.** 규격이 같으니 `is_out_of_spec` 결과가 그대로입니다.
