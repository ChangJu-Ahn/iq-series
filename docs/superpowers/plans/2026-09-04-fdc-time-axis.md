...
### 노트북 실행 검증을 저장소로 들여왔다 (커밋 `97a5494`)

작업 내내 노트북 셀을 실제로 실행한 것은 `/tmp` 의 임시 하네스뿐이었다. 커밋된
`test_build_notebook.py` 51개는 셀 **소스**만 본다 — 문자열 포함, AST 파싱, 모듈
정의 순서. 전부 필요하지만 셀을 한 번도 실행하지 않는다.

그 하네스가 OS 의 `/tmp` 정리로 사라졌다. 뒤늦게 확인해 보니 커밋된 테스트만으로는
**구문은 맞고 실행이 깨지는** 종류를 잡지 못하는 상태였다.

리뷰 다섯 라운드가 전부 "에러가 안 나는 실패" 였다. 이건 그 반대다. 에러는 나는데
참가자 20명 앞에서 난다. 이 모듈의 존재 이유가 "각자 자기 Fabric 에서 이 노트북을
돌린다" 인 만큼, 안 돌아가는 노트북이 최악의 실패 모드다.

`tests/test_notebook_execution.py` 는 Fabric 이 주는 것(`spark`, `mssparkutils`)과
바깥 세계(MES HTTP, Kusto)를 대역으로 세우고 코드 셀을 순서대로 `exec` 한다. 대역은
얇게 만들었다 — KQL 을 흉내 내기 시작하면 대역이 곧 두 번째 구현이 되고, 그때부터
대역의 버그와 노트북의 버그를 구별할 수 없다. 3차에서 PySpark 를 복제하다 걸린
함정과 같은 것이다.

시나리오 8개는 다섯 라운드가 지적한 것들을 **실행 경로에서** 되짚는다.

| 시나리오 | 되짚는 라운드 |
|---|---|
| 첫 실행이 MES 구간 시작부터 백필 | 계획서의 벽시계 백필 경고 |
| 두 번째 실행은 증분 + 스펙 건너뜀 | — |
| 비UTC 드라이버의 naive watermark | 3차 |
| 미래 watermark 는 멈춘다 | 3차 |
| 조회 실패는 백필로 안 떨어진다 | 1차 |
| 스펙 부분적재 / 중복은 멈춘다 | 2차 · 4차 |
| 쓰기 옵션이 실제로 붙는다 | 5차 |

마지막 하나는 문자열 검사가 원리상 못 하는 일을 한다. 옵션이 `if` 안에 들어가 안
불리게 돼도 소스 검사는 통과하지만, 대역이 받은 것을 보면 잡힌다.

돌연변이 8종으로 검증했다 — `astimezone`→`replace` / 미래 가드 제거 / 조회실패
폴백 / 스펙 게이트 뭉개기 / `timeZone` 제거 / `Append`→`Overwrite` / 벽시계 백필 /
증분 무력화. 전부 **해당 시나리오가** 잡는다. **352 테스트.**
