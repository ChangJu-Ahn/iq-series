...
그런데 FDC 판독은 평행이동에 불변이 아니다.

```
              원본      +37시간   +11일
판독 행       102,552   102,552   102,552   ← 같음
Run / Idle    93,210    93,210    93,210    ← 같음
Alarm 행         301       240       310    ← 다름
경보 난 설비        7         6         6    ← 다름
```

`diurnal()` 이 `moment.hour` 를 쓴다. 하루 주기라 밤 공정이 새벽 공정이 되면 기저가 이동해
임계를 넘나드는 지점이 바뀐다. **물리적으로 맞는 동작이라 고칠 것이 아니다.** 20명이 각자
돌리면 강사의 "IMPL01 에서 경보 났죠"에 답이 갈린다. 드리프트 가드로는 못 막는다 — 각자는
내부 정합이 맞기 때문이다. 앵커 고정만이 답이다.

**계획서에 이 항목이 없었던 이유**: Task 5 를 `span(FACTS)` 로 유도하게 설계해서 앵커
독립이라고 믿었다. 구간은 독립인데 **구간 안의 절대 시각**은 아니다. 상대 설계가 절대
의존을 다 없애지 못한다는 것을 구분하지 않았다.

### 계획에 없던 도구 — `mutants.py`

검증 항목이 늘면서 "테스트가 실제로 잡는가"를 매번 손으로 확인하게 됐다. 재사용 도구로
만들었다. 14종을 정의하고 네 가지 조용한 실패를 가른다.
...
**MES 앵커 고정.** bicep 재배포 없이 한 줄로 된다.

```bash
az containerapp update -n mock-mes -g <리소스그룹> \
  --set-env-vars MES_ANCHOR=2026-09-15T00:00:00Z
```

픽스처는 평행이동이라 그대로 유효하다. 다시 뜰 필요 없다.

실 Fabric 미검증분은 그대로다 — `maxif` 의 빈 `datatable` 레그 동작,
`GenerateDynamicCsvMapping`, 실행 겹침.
