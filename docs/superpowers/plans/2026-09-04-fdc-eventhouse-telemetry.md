...
만들었다. 19종을 정의하고 여섯 가지 조용한 실패를 가른다.
...
| 복원 후 산출물 미복원 | 다음 돌연변이가 엉뚱하게 실패 | 이번 |
| **지표 불변** | **다섯 다 통과인데 돌연변이가 안 닿음** | **이번** |
...
여섯째는 약한 누수를 만들려다 나왔다. 이탈 센서 선택을 5% 확률로 비틀었는데 불량 런
31개 중 2개만 걸렸고 그마저 다른 설비여서 무지목 경보가 그대로였다. 적용됐고 문자열도
바뀌었고 크래시도 아니다. **테스트 결과를 읽기 전에 돌연변이가 의도한 일을 했는지부터
봐야 한다.** probe 가 `값 (원래 N)` 형식으로 적으므로 짝지어 대조해 "무효" 로 가른다.

### 전제 오류 5 — "한계를 세웠으니 그물이 생겼다"

QMS 가 "지표는 정확히 움직였는데 `3 <= 3` 으로 걸쳐 통과했다"고 알려 와서 같은 자리를
찾았다. `assert actual <= max(3.0, expected * 8)` 이 실측 2 와 여유 1건이었다. 고치려고
기대값 모델부터 봤더니 **모델이 2,095배 어긋나 있었다.**
...
`AMBIENT_TEMP` 는 하루 주기 진폭이 정상범위 반폭의 60% 라 잡음에 남은 여유가 3.33σ
뿐인데, 주석은 "정상 범위는 4.17~10σ" 라고 적고 있었다. 환경 성분을 빼고 센 숫자였다.

포아송 꼬리로 한계를 다시 유도해 넣고 통과를 봤다. 그런데 한계값을 출력해 보니 **전부
1000** — 루프 상한이었다.

```python
limit, tail, term = 1, math.exp(-mean), math.exp(-mean)   # tail 이 P(X=0) 이다
```

쉘에서 검증할 때는 `tail = 1 - term` 이었는데 옮기면서 `1 -` 가 사라졌다. 꼬리가 아니라
머리에서 시작하니 아무리 빼도 예산 아래로 안 내려간다. **한계가 걸친 게 아니라 그물이
아예 없었고, 통과를 보고 넘어갈 뻔했다.**

돌연변이를 건 뒤 지표를 보라는 규칙은 있었는데 **테스트를 새로 쓴 뒤 그 테스트가 유도한
한계값을 출력해 보는** 규칙은 없었다. `assert limit < 1000` 으로 수렴 실패를 결함으로
세웠다.

### 개수가 아니라 자리

QMS 의 "clamp 의 지문은 개수가 아니라 자리" 를 적용해 무지목 주의가 **어디에** 놓이는지
7개 시프트로 쟀다.

| 시프트 | 0h | 1h | 7h | 13h | 37h | 264h | 720h |
|---|---|---|---|---|---|---|---|
| 총 건수 | 2 | 1 | 1 | 0 | 0 | 2 | 2 |
| 짝당 최대 | 1 | 1 | 1 | 0 | 0 | 1 | 1 |

총 건수는 0~2 로 흔들리는데 짝당 최대는 언제나 1 이다. 잡음은 독립이라 같은 짝에 두 번
오기 어렵고 이탈은 지속되므로 쌓인다. 누수 돌연변이를 걸면 짝당 최대가 1 → **1,175** 로
튄다. 짝별 포아송 한계를 따로 세웠다.

짝당 2~5건짜리 약한 누수는 여전히 통과한다. 잡으려면 한계를 3 쪽으로 내려야 하는데
그러면 정상 변동에 걸린다. 진짜 이탈은 지속되므로 수천 건이고 5건짜리 누수를 만들
물리적 경로가 없다. **무해로 판정했다** — 경고할 이유가 없는 것까지 잡으려고 그물을
조이면 나중에 정당한 변경이 걸린다.

### 도구를 검증하는 도중 도구가 남의 작업을 지웠다

`_probe_moved` 가 그물인지 확인하려고 치환문자열을 어긋내고 되돌리면서 `git checkout --
mutants.py` 를 썼다. 그때 `python3 mutants.py` 가 백그라운드로 돌고 있었다. 돌연변이는
대상 파일을 고쳤다 되돌리는 중이라 `git status` 가 더러웠고, `checkout` 은 **그 순간
작업 중이던 편집을 통째로 지웠다.** 되돌리기는 백업 파일로 해야 한다.

### MES 최종 확정 — 앵커가 6월로 옮겨졌다

MES 가 마무리되어 라이브를 다시 찍었다.

```
앵커     2026-06-13T00:00:00+00:00     (이전 09-01)
구간     2026-06-10 07:13 ~ 06-13 00:00 (64.8h)
공정결과 91행 · 설비 8 · 불량 런 35건
런 길이  최소 0.27h · 중앙 1.28h · 최대 3.85h · 0 이하 없음
```

`in_time`/`out_time` 간격이 들어갔다. 상대구조 해시(`bd13ecc916a21a6b`)는 픽스처와
**동일** 하다 — 데이터는 그대로고 앵커만 83일 옮겨졌다.

```
판독 102,552행 전수 비교
  값 같은 행      102,552 (100.0%)
  상태 같은 행    102,552
  경보 설비       CMP01·CVD01·DIFF01·ETCH01·PHOT02 (양쪽 동일)
  시각 외 다른 칸  없음
  시각은 정확히 83일 밀림
```

83일이 정수 일수라 잡음 시드 수정이 그대로 작동했다. **픽스처는 갈지 않는다** — 갈면
"정수 이동에는 값이 변하지 않는다"를 실증하는 대조군이 사라진다.

QMS 가 제보한 "문자열 안에 박힌 날짜"(`supplier_lot_no` 같은 것)는 이 전수 비교에서
시각 외 다른 칸이 하나도 없어 해당 없음을 확인했다. 판독 행의 문자열 컬럼은 `eqp_id` ·
`eqp_type` · `step_code` · `run_status` · `sensor_code` · `unit` · `status` 뿐이다.

확인 과정에서 비교 방법 자체가 틀릴 뻔했다. 위치로 `zip` 해서 재면 `+1시간 7분` 에서
`sensor_code` 가 59,114칸 다르게 나온다. 결함이 아니라 **행 순서가 어긋나 엉뚱한 짝을
비교** 한 것이다. `(eqp_id, sensor_code, reading_ts - 이동폭)` 로 짝지으면 시각 외 차이가
0 이다. 시각이 정확히 밀렸는지부터 확인하고, 안 밀렸으면 위치 비교를 버려야 한다.

그러면서 분·초 단위 이동의 성질이 드러났다.

| 이동 | 행 수 | 짝지음 | 짝 없음 |
|---|---|---|---|
| +37시간 (정시) | 102,552 | 102,552 | 0 |
| +1시간 7분 | 102,528 | 93,210 | 9,318 |
| +30초 | 102,542 | 93,210 | 9,332 |

짝지어진 93,210 은 Run 판독 수와 정확히 같다. **Idle 판독만 격자가 재정렬된다.** Idle 은
런 사이를 벽시계 격자로 채우므로 앵커가 분·초 단위로 움직이면 격자점이 달라지고 행 수도
±24 변한다. 실제 장비도 벽시계로 샘플링하니 물리적으로는 맞는 동작이고, 앵커를 고정하면
일어나지 않는다. 다만 **앵커가 풀려 재시작 시각(초 단위)이 앵커가 되면 값뿐 아니라 행
수까지 달라진다** — 앵커 고정 경고의 근거가 하나 늘었다.
